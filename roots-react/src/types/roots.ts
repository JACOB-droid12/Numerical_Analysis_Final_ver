...
export interface FixedPointRankingCandidate {
  id: string;
  label: string;
  gExpression: string;
}

export interface FixedPointRankingConfig {
  target: string;
  targetValue: number;
  p0: string;
  candidates: FixedPointRankingCandidate[];
}
...
export type FixedPointRankingStatus =
  | 'converged'
  | 'diverged'
  | 'cycle'
  | 'undefined'
...
export interface FixedPointRankingRow {
  candidate: FixedPointRankingCandidate;
  status: FixedPointRankingStatus;
  rank: number | null;
  iterations: number;
...
export interface FixedPointRankingResult {
  presetId: string;
  target: string;
  targetValue: number;
  p0: string;
  rows: FixedPointRankingRow[];
}
